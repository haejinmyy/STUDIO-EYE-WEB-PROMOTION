...
export default function FAQCEditPage() {
  ...
  // 글자수 제한
  const [questionLength, setQuestionLength] = useState(clickedFAQ && clickedFAQ.question.length);
  ...
  const onValid = (data: IFAQData) => {
    ...
    if (!checkIsEmpty(editorState, 'Answer') && window.confirm('수정하시겠습니까?')) {
      axios
        .put(`${PROMOTION_BASIC_PATH}/api/faq`, formData)
        .then((response) => {
          alert('FAQ가 수정되었습니다.');
          navigator(`${PA_ROUTES.FAQ}/${clickedFAQ && clickedFAQ.id}?page=${currentPage}`);
        })
        .catch((error) => {
          console.log(error);
  ...
  const handleDelete = (id: number) => {
    if (window.confirm('삭제하시겠습니까?')) {
      ...
      navigator(`${PA_ROUTES.FAQ}?page=1`);
    }
  };
